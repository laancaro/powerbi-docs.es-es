...
Las características disponibles dependen de si es el *creador* (persona que realizó el mapa) o el *consumidor* (alguien ha compartido un mapa de ArcGIS con usted). Si interactúa con un mapa de ArcGIS como consumidor (también conocido como [Vista de lectura](end-user-reading-view.md)), estas son las acciones disponibles.

* Si es un consumidor Premium con permisos de *vista*, podrá [ver los datos utilizados para crear la visualización](end-user-show-data.md), [suscribirse](end-user-subscribe.md), ver el mapa en [modo de enfoque y modo de pantalla completa](end-user-focus.md), [visualizar el contenido relacionado](end-user-related.md), [interactuar con los filtros](end-user-report-filter.md) establecidos por el *creador del informe*, [compartir el informe](../service-share-reports.md) y mucho más.
