...
![rastrear desagrupando datos un nivel](./media/end-user-drill/power-bi-drill-icon4.png)  ![activar y desactivar rastrear desagrupando datos](./media/end-user-drill/power-bi-drill-icon2.png)  ![icono de rastrear desagrupando datos en todos los campos a la vez](./media/end-user-drill/power-bi-drill-icon3.png)
![icono de rastrear agrupando datos](./media/end-user-drill/power-bi-drill-icon5.png) ![icono de expandir](./media/end-user-drill/power-bi-drill-icon6.png)  
...
2. Para seguir el tutorial, [abra el ejemplo Retail Analysis](../sample-datasets.md) y cree un mapa de árbol que examine **Total Units This Year** (Values) [Unidades totales este año (Valores)] por **Territory** (Territorio), **City** (Ciudad), **Postal Code** (Código postal) y **Name** (Group) [Nombre (Grupo)].  El mapa de árbol tiene una jerarquía formada por el territorio, la ciudad, el código postal y el nombre de la ciudad. Cada territorio tiene una o varias ciudades, cada ciudad tiene uno o más códigos postales, etc. De forma predeterminada, la visualización muestra solo los datos de territorio, porque *Territory* (Territorio) aparece en primer lugar en la lista.
   
   ![Seleccione "Territory" (Territorio).](media/end-user-drill/power-bi-hierarcy-list.png)

2. Como puede resultar confuso intentar comprender cómo funcionan entre sí los distintos iconos del modo detallado, vamos a filtrar el mapa de árbol para mostrar solo dos de los territorios más pequeños: **KY** y **TN**. Seleccione el mapa de árbol y, en **Filtros de nivel visual** expanda **Territory** (Territorio) y seleccione **KY** y **TN**.

    ![filtro para KY y TN](./media/end-user-drill/power-bi-filter.png)    

   Ahora solo se muestran dos territorios en el mapa de árbol.
...
- En la barra de menús de Power BI, seleccione el botón **Explorar**.
   ...
   ![La selección de Explorar muestra iconos y opciones de exploración](media/end-user-drill/power-bi-explore.png)
...
***Expandir*** agrega un nivel de jerarquía adicional a la vista actual. Por tanto, si está mirando el nivel **Territory** (Territorio), puede expandir y agregar City (Ciudad), Postal Code (Código postal) y Name (Nombre) al mapa de árbol. Cada paso de la ruta muestra la misma información y agrega un nivel de información nueva.
...
1. Empiece en el nivel superior del mapa de árbol, que muestra datos de KY y TN. Amplíe el mapa de árbol seleccionando uno de los controladores y arrastrándolo a la derecha. 

    ![mapa de árbol que muestra dos estados](./media/end-user-drill/power-bi-drill-down.png) .

2. Para rastrear desagrupando datos de ***todos los campos a la vez***, seleccione la flecha doble de la esquina superior izquierda de la visualización ![icono de doble rastreo desagrupando datos](./media/end-user-drill/power-bi-drill-icon3.png). El mapa de árbol muestra ahora los datos de ciudad de Kentucky y Tennessee. 

    ![icono de doble modo detallado](./media/end-user-drill/power-bi-drill-down1.png)
   
5. Rastree desagrupando datos una vez más en el nivel Postal Code (Código postal) de la jerarquía.

    ![icono de doble modo detallado](./media/end-user-drill/power-bi-drill-down2.png)

3. Para volver a rastrear agrupando datos, seleccione la flecha ascendente situada en la esquina superior izquierda de la visualización ![icono de rastrear agrupando datos de un nivel](./media/end-user-drill/power-bi-drill-icon5.png).
...
1. Seleccione el icono de rastrear desagrupando datos para activar esta opción ![rastrear desagrupando datos activada](./media/end-user-drill/power-bi-drill-icon2.png). Ahora tiene la opción de rastrear desagrupando datos de ***un solo campo a la vez***. 
   ...
   ![flecha que apunta a la activación o desactivación de rastrear desagrupando datos](media/end-user-drill/power-bi-drill-icon-new.png)
...
Tener un mapa de árbol que nos muestra solo un código postal no es muy informativo.  Así que vamos a expandir un nivel en la jerarquía.  

1. Con el mapa de árbol activo, seleccione el icono de *expandir* ![icono de expandir](./media/end-user-drill/power-bi-drill-icon6.png). El mapa de árbol muestra ahora dos niveles de la jerarquía: Postal Code (Código postal) y Name (Nombre) de la tienda. 
...
![Barra que muestra los datos agrupados por año](media/end-user-drill/power-bi-hierarchical-axis-datetime-1.png)

Con el modo de rastrear desagrupando datos activado, haga clic en la barra del gráfico para bajar un nivel de la jerarquía. Verá tres barras para los datos de los trimestres disponibles. Entonces, en los iconos izquierdos superiores, elija **Expand all down one level of the hierarchy** (Expandir hacia abajo un nivel de la jerarquía). Luego vuelva a hacerlo para ir al nivel inferior de la jerarquía, que muestra resultados de cada mes.

![gráfico de barras para ver la barra por mes](media/end-user-drill/power-bi-hierarchical-axis-datetime-2.png)
...
Para este ejemplo, continúe con el [ejemplo Retail Analysis](../sample-datasets.md). Cree una visualización de gráfico de columnas apiladas que examine **Total Units This Year** (valores) por **Territory**, **City**, **Postal Code** y **Name** (grupo).  

![gráfico de barra que muestra las unidades totales de este año por territorio](media/end-user-drill/power-bi-hierarchical-axis-category-1.png)

Con el modo de rastrar desagrupando datos activado, en los iconos izquierdos superiores, elija **Expand all down one level of the hierarchy** (Expandir hacia abajo un nivel de la jerarquía) tres veces.
Debería encontrarse en el nivel inferior de la jerarquía, que muestra los resultados de Territory, City y Postal Code.

![gráfico de barras que muestra el nivel más bajo de la jerarquía, nivel más detallado](media/end-user-drill/power-bi-hierarchical-axis-category-2.png)
...
* Si al agregar un campo de fecha a una visualización no se crea una jerarquía, es posible el campo de fecha no se guarde realmente como una fecha. Si es el propietario del conjunto de datos, ábralo en la vista *Datos* de Power BI Desktop, seleccione la columna que contiene la fecha y, en la pestaña Modelado, cambie el **Tipo de datos** a **Fecha** o **Fecha/hora**. Si el informe se ha compartido con usted, póngase en contacto con el propietario para solicitar el cambio.  
  ...
  ![Seleccione la vista de datos y, en la esquina superior derecha, vea el Tipo de fecha](media/end-user-drill/power-bi-change-data-type2.png)
