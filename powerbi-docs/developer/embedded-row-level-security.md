...
Si va a realizar una inserción para usuarios que no son de Power BI (la aplicación posee los datos), que es normalmente un escenario de ISV, este es el artículo que necesita. Debe configurar el token de inserción para tener en cuenta el usuario y el rol. Siga leyendo para saber cómo hacerlo.
...
4. Para asegurarse de que las reglas funcionen, en la pestaña **Modelado**, seleccione **Ver como roles** y, luego, el rol **Manager** que acaba de crear, junto con **Otros usuarios**. Escriba **AndrewMa** como usuario.
...
La API acepta una lista de identidades con la indicación de los conjuntos de datos pertinentes. Para que RLS funcione, debe pasar lo siguiente como parte de la identidad.
